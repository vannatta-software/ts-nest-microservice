...
export class RabbitMQEventBus extends EventBus implements OnModuleInit, OnModuleDestroy {
    ...
    private subscriptions: Map<string, (msg: any) => Promise<void>> = new Map();
    ...
    private async setupSubscription<TData>(topicName: string, handler?: (data: TData) => Promise<void>, eventType?: ClassType<TData>): Promise<void> {
        ...
        await this.channel.consume(topicName, messageHandler, { consumerTag });
        this.subscriptions.set(topicName, messageHandler);
        this.logger.debug(`RabbitMQ subscribed to topic: ${topicName}`);
    }

    async onModuleDestroy() {
        for (const [topicName, handler] of this.subscriptions.entries()) {
            // No explicit unsubscribe method in amqplib, closing channel will stop consumers
            this.logger.log(`Stopping consumer for topic: ${topicName}`);
        }
